...
public class ProductDaoSqls {
    ...
    public static final String SELECT_COUNT = "SELECT count(*) FROM product p INNER JOIN product_image img on p.id = img.product_id and img.type = \"th\" INNER JOIN file_info f on img.file_id = f.id INNER JOIN display_info d on p.id = d.product_id";
    public static final String SELECT_COUNT_BY_CATEGORY_ID = "SELECT count(*) FROM product p INNER JOIN product_image img on p.id = img.product_id and img.type = \"th\" INNER JOIN file_info f on img.file_id = f.id INNER JOIN display_info d on p.id = d.product_id where p.category_id = :categoryId";
}
